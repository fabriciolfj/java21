package com.github.fabriciolfj.examplemapmulti;
...
public class Simulacao {
    ...
    static void main() {
        var integers = List.of(3, 2, 5, 6, 7, 8);
        List<Integer> evenDoubledMM = integers.stream().<Integer>mapMulti((i, consumer) -> {
            if (i % 2 == 0) {
                consumer.accept(i * 2);
            }
        }).toList();

        System.out.println(evenDoubledMM);

        int evenDoubleAndSumMM = integers.stream().mapMultiToInt((i, consumer) -> {
            if (i % 2 == 0) {
                consumer.accept(i * 2);
            }
        }).sum();

        System.out.println(evenDoubleAndSumMM);
